...
select * from BOMON --lấy hết dữ liệu trong table bộ môn

go
select MABM, TENBM, DIENTHOAI from BOMON

--Đổi tên hiển thị
go
select MABM as N'Mã Bộ Môn', TENBM as N'Tên Bộ Môn', DIENTHOAI as N'Số điện thoại' from BOMON

Select BM.MABM, BM.TENBM, GV.HOTEN, GV.LUONG from BOMON as BM, GIAOVIEN as GV

--Select có điều kiện
Select * from BOMON
where MAKHOA = 'HH' and PHONG = 'B44'